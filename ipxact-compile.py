...
MES_SEV_WAR = 1
MES_SEV_ERR = 2
def message(severity, text):
	if severity == MES_SEV_INF:
		print('information: ' + text)
	elif severity == MES_SEV_WAR:
		print('warning: ' + text)
	elif severity == MES_SEV_ERR:
		print('error: ' + text)
		sys.exit(1)


if not (sys.version_info.major >= 3 and sys.version_info.minor >= 8):
	message(MES_SEV_ERR, 'python minimum version 3.8 required for xml.etree.ElementTree findall() support for {*} but found python version ' + '.'.join(map(str, sys.version_info[:2])))
...
name = 'name'
#message(MES_SEV_INF, 'find ' + name)
nodes = tree.findall('{*}' + name)
if len(nodes) == 0:
	message(MES_SEV_ERR, 'xml contains no top <' + name + '></' + name + '> element')
elif len(nodes) > 1:
	message(MES_SEV_ERR, 'xml contains multiple top <' + name + '></' + name + '> elements')
else:
	top_name = nodes[0].text
	#message(MES_SEV_INF, 'top name = ' + top_name)
...
if len(nodes) == 0:
	...
	#message(MES_SEV_INF, 'found one top ' + name)
...
if len(nodes) == 0:
	message(MES_SEV_ERR, 'xml contains no <' + name + '></' + name + '> element')
elif len(nodes) > 1:
	message(MES_SEV_WAR, 'xml contains multiple <' + name + '></' + name + '> elements, will use the first one')
	node_fileset = nodes[0]
else:
	node_fileset = nodes[0]
	#message(MES_SEV_INF, 'found one ' + name)
...
if len(nodes) == 0:
	...
	message(MES_SEV_ERR, 'xml contains multiple <' + name + '></' + name + '> elements')
else:
	fileset_name = nodes[0].text
	#message(MES_SEV_INF, 'fileset name = ' + fileset_name)
...
if len(nodes) == 0:
	...
	for file_node in file_nodes:
		...
		if len(nodes) == 0:
			message(MES_SEV_ERR, 'file contains no <' + name + '></' + name + '> element')
		elif len(nodes) > 1:
			message(MES_SEV_ERR, 'file contains multiple <' + name + '></' + name + '> elements')
		else:
			file_name = nodes[0].text
			#message(MES_SEV_INF, 'file name = ' + file_name)
		...
		if len(nodes) == 0:
			...
			#message(MES_SEV_INF, 'file type = ' + file_type)

		name = 'logicalName'
		nodes = file_node.findall('./{*}' + name)
		if len(nodes) == 0:
			#message(MES_SEV_INF, 'file contains no <' + name + '></' + name + '> element')
			file_logical = None
		elif len(nodes) > 1:
			message(MES_SEV_ERR, 'file contains multiple <' + name + '></' + name + '> elements')
		else:
			file_logical = nodes[0].text
			#message(MES_SEV_INF, 'file logical name = ' + file_logical)
...
def tool_verilator(files, compile_options):
	message(MES_SEV_INF, 'output compile script for verilator')
	...
	for file in files:
		...
		if file[1] in ['verilogSource', 'verilogSource-95', 'verilogSource-2001', 'systemVerilogSource', 'systemVerilogSource-3.0', 'systemVerilogSource-3.1', 'systemVerilogSource-3.1a']:
			...
			message(MES_SEV_WAR, 'ipxact-compile does not know if verilator supports file type: ' + file[1] + ' for file: ' + file[0])
	return [compile_command]
...
if arguments.tool == 'example':
	...
	message(MES_SEV_ERR, 'unknown tool: ' + arguments.tool)

if output != None:
	with open(arguments.output, 'w') as file_output:
		message(MES_SEV_INF, 'write output file: ' + arguments.output)
		file_output.write('\n'.join(output) + '\n')
